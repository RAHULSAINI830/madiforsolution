...
(function ($) {
  ...
  async function reloadAjax($off, $el = $(document)) {
    if (!$off) {
      ...
      window.$animate = await p2nGrid.effectAnimate();
    }

    await $('.hidden[data-p2n="bg_section"]').each(function () {
    ...
    if (!$body.hasClass('elementor-editor-active')) p2nAjax() && (await p2nAjax().start());
  ...
  function imgPosition() {
    $("[data-p2n-position]").each(function () {
      if (this.nodeName === "IMG") $(this).css("object-position", p2nGrid.getData(this, "position", "center"));else $(this).css("background-position", p2nGrid.getData(this, "position", "center"));
    });
  }

  function gridGaps() {
    $(".d-grid[data-p2n-gap]").each(function () {
      const gap = p2nGrid.getData(this, "gap", "30px 30px");
      const split = gap.split(" ");
      this.style.gridGap = gap;
      this.style.setProperty("--grid-gap", gap);

      if (this.classList.contains('p2n-isotope')) {
        if (split.length > 1) {
    ...
    $("[data-p2n-iconsize]").each(function () {
      this.style.setProperty("--p2n-icon-size", p2nGrid.getData(this, "iconsize"));
    });
  }
  /**
   *
  ...
  function tabs($el) {
    $el.find(".p2n-tabs").each(function () {
      const $this = $(this);
  ...
  function Headline($el) {
    if ($.isFunction($.fn.animatedHeadline)) {
      $el.find('.ah-headline:not(.p2n-active-line)').animatedHeadline();
      $el.find('.ah-headline').addClass('p2n-active-line');
    }
  }

  function accordion($el = $(document)) {
    $el.find(".p2n-accordion").each(function () {
      const $this = $(this),
  ...
  function initMap($el = $(document)) {
    ...
    if (mapScriptId === null) {
      const GOOGLE_MAP_KEY = p2nParam.map.api,
    ...
    setTimeout(function () {
      try {
        map_id.each(function () {
          let map_att = $(this),
              lat = p2nGrid.getData(this, "lat"),
              leg = p2nGrid.getData(this, "len"),
              zoom = p2nGrid.getData(this, "zoom");
  ...
  function preloader() {
    ...
    const progress_number = preloader.find(".loading-count"),
          preloader_progress = preloader.find('.p2n-progress-path'),
          present = {
      value: 0
    };
    ...
    window.addEventListener('DOMContentLoaded', function () {
      ...
      tl.to(present, 1, {
        ...
        attr: {
          d: p2nGrid.getData(preloader.find('.bg-load .separator__path').get(0), 'to')
        },
  ...
  function effectBackForward() {
    $wind.on("popstate", function (e) {
      ...
      if (window.location.hash.length) {
        $wind.scrollTop(0);
        p2nGrid.scrollTop(window.location.hash, 1, -100);
        return;
      }
      ...
      setTimeout(function () {
        p2nAjax().backAnimate(e);
      }, 50);
    });
  }

  function p2nAjax() {
    return p2nGrid.p2nAjax({
      className: 'p2n-ajax-effect',
  ...
  function menuInit() {
    ...
    const targets = {
      ...
      backgroundMain: menu.querySelector(".bg-load:not(.p2n-svg-transition)"),
      svg: menu.querySelector("svg.bg-load path"),
      subMenu: $(menu).find("li.nav-item.has-sub-menu > a"),
      back: $(menu).find("li.p2n-back"),
      hamburger: menu.classList.contains('p2n-hamburger'),
      scrDown: 0
    };
    const reserved = p2nGrid.useState(false, (newValue, oldValue) => oldValue && removeOpenMenu());
    const typeNav = p2nGrid.useState(targets.hamburger, newValue => newValue ? menu.classList.add("p2n-hamburger") : menu.classList.remove("p2n-hamburger"));
    ...
    const onCompleteAnimate = e => {
      ...
      menu.classList.toggle('p2n-open');
    ...
    const toggleClick = function () {
      if (!reserved.getValue()) {
        p2nGrid.svgAnimate.up(targets.svg, TransEnd).to("#p2n-scrollbar", {
          y: -200,
          duration: 1,
          ease: 'power4.in'
      ...
      } else p2nGrid.svgAnimate.down(targets.svg, () => onCompleteAnimate(this)).to("#p2n-scrollbar", {
        y: 0,
        clearProps: "y",
        duration: 1,
        ease: 'power4'
    ...
    new Promise(resolve => setTimeout(() => resolve(), 300)).then(() => {
      ...
      return p2nGrid.spltting.Char((_targets$toggle = targets.toggle) === null || _targets$toggle === void 0 ? void 0 : _targets$toggle.querySelector('.text-menu'));
    }).then(() => {
      var _targets$toggle2;

      return p2nGrid.spltting.Char((_targets$toggle2 = targets.toggle) === null || _targets$toggle2 === void 0 ? void 0 : _targets$toggle2.querySelector('.text-open'));
    }).then(() => {
      var _targets$toggle3;

      return p2nGrid.spltting.Char((_targets$toggle3 = targets.toggle) === null || _targets$toggle3 === void 0 ? void 0 : _targets$toggle3.querySelector('.text-close'));
    }).then(() => {
      targets.back.find(".text-toggle-back").each(function ($index) {
        setTimeout(() => p2nGrid.spltting.Char(this), 10 * $index);
      });
    }).then(() => {
      menu.querySelectorAll('ul').forEach((item, index) => {
        item.style.setProperty('--p2n-li-name', "p2n" + index);
        Object.keys(item.children).forEach($key => {
          item.children[$key].style.setProperty('--p2n-li-index', $key);
        });
      });
    }).then(() => {
      gsap.set(menu, {
  ...
  function dropHash() {
    ...
    linked.scroll.on("click", function (e) {
      ...
      const option = $(this).data('p2n-option');
      p2nGrid.scrollTop(href.get(0).offsetTop, option);
      href = null;
    });

    if (window.location.hash.length) {
      $wind.scrollTop(0);
      p2nGrid.scrollTop(window.location.hash);
    }
  ...
  function scrollBarWidth() {
    ...
    document.body.style.setProperty('--p2n-width-scroll', scrollDiv.offsetWidth - scrollDiv.clientWidth + "px");
  ...
  async function Isotope($el = $(document)) {
    ...
    await $el.find('.root-posts').each(function () {
      const rootPosts = $(this),
            $filtering = rootPosts.find('.p2n-filtering .filtering'),
            $isoItem = rootPosts.hasClass('has-filter') ? rootPosts.find('.p2n-posts') : rootPosts.find('.p2n-isotope'),
      ...
      const handleAjax = function ($iso) {
        ...
        const $option = p2nGrid.getOptionAjax($buttonAjax.get(0));
        p2nGrid.loadMore({
          el: $buttonAjax,
          option: $option,
          isotope: $iso,
          ...
          filtering: $filtering,
          posts: rootPosts.find('.p2n-posts')
        });
        return $iso;
      };
    ...
    await $el.find(".p2n-isotope:not(.use-filter)").each(function () {
  ...
  function loadLazyImage($el = $(document), $type, $args = {}) {
    if (typeof $type === 'object') {
      ...
      const p2nData = $el.find("[data-p2n-" + $type + "]");
      p2nData.each(function ($index) {
        const $args = {};
        if (p2nData.length - 1 === $index && $type === "src") $args.onComplete = () => {
        ...
        setTimeout(() => {
          p2nGrid.loadData(this, $type, $args);
        }, 1000);
      });
    }
  }
  ...
  function serviceSection($el = $(document)) {
    if (p2nGrid.isMobile()) return;
    ($el.hasClass("service-with-img") ? $el.find('.p2n-service') : $el.find(".service-with-img")).each(function () {
  ...
  function effectScroller() {
    ...
    return {
       ...
       */
      isScroller: function () {
        var _p2nParam$scrollbar;

        return p2nParam === null || p2nParam === void 0 ? void 0 : (_p2nParam$scrollbar = p2nParam.scrollbar) === null || _p2nParam$scrollbar === void 0 ? void 0 : _p2nParam$scrollbar.smooth;
      },
      start: function () {
        ...
        lenisScroll = new Lenis(p2nParam.scrollbar);
  ...
  function compareTowImage($el = $(document)) {
    $el.find('.p2n-compare-container').each(function () {
      const compare = p2nGrid.compareTowImg(this);
      p2nGrid.killAjax(function () {
        compare.destroy();
      });
    });
  ...
  function projectSlider() {
    return {
      ...
      run: function () {
        let $this = this;
        $(".p2n-swiper").each(function () {
          let option = p2nGrid.getData(this, "option", {});
          let syn = $(this).parent().find(p2nGrid.getData(this, "controller"));
  ...
  function justifiedGallery($el = $(document)) {
    $el.find(".gallery-portfolio").each(function () {
      ...
      const option = p2nGrid.getData(this, 'option') || {};
  ...
  function sliderPortfolio($el = $(document)) {
    ...
    const initSlider = async function () {
      const p2nSliderContent = this.querySelector('.p2n-slider-content');
      await this.querySelectorAll('.content-slider .slide-item').forEach(($item, $index) => {
        ...
        $item.setAttribute('data-p2n-id', $index);
        if (!slideContent) return;
        slideContent === null || slideContent === void 0 ? void 0 : slideContent.setAttribute('data-p2n-id', $index);
        if ($index === 0) slideContent === null || slideContent === void 0 ? void 0 : slideContent.classList.add('p2n-active', 'p2n-active-animate');
        ...
        p2nSliderContent === null || p2nSliderContent === void 0 ? void 0 : p2nSliderContent.append(slideContent);
      });
      ...
      await this.querySelectorAll('.p2n-slider-content .slide-content [data-p2n-split=\"chars\"]').forEach(function ($item) {
        p2nGrid.spltting.Char($item, true);
      });
      await this.querySelectorAll('.p2n-slider-content .head-meta').forEach(function ($item) {
        p2nGrid.spltting.Items($item, "span");
      });
    };
    ...
    const getContent = (swiper, contentRef) => {
      const oldNum = swiper.slides[swiper.previousIndex].getAttribute("data-p2n-id");
      const newNum = swiper.slides[swiper.activeIndex].getAttribute("data-p2n-id");
      return [newNum, oldNum, contentRef.querySelector('[data-p2n-id="' + newNum + '"]'), contentRef.querySelector('[data-p2n-id="' + oldNum + '"]')];
    };

    function slideChange(swiper) {
      const contentSlider = this.querySelector('.p2n-slider-content');
      ...
      if (swiper.p2nOnChange) swiper.p2nOnChange(newNum, oldNum, newContent, oldContent);
      tl.progress(1);
      tl = new gsap.timeline();
      oldContent.classList.remove("p2n-active-animate");
      tl.fromTo(oldTitle, 0.4, animate.show, animate.hide).call(function () {
        oldContent.classList.remove("p2n-active");
        newContent.classList.add("p2n-active");
        newContent.classList.add("p2n-active-animate");
    ...
    const NavSwiper = function (container, swiper) {
      ...
      if (window.innerWidth > 575) {
        navContainer.querySelectorAll('h6.sm-title-block').forEach($item => {
          const s = p2nGrid.spltting.Char($item);
      ...
      $(this).find(".bg-container .slide-item").each(function () {
        ...
        if (slide_content.find('video').length) {
          images[id] = {
            ...
            src: slide_content.find('video').attr("data-p2n-poster"),
            overlay: slide_content.data("overlay")
          };
        } else {
          const img = slide_content.find('img');
          const srcImg = img.data('p2n-src');
          images[id] = {
    ...
    $el.find(".main-slider:not(.p2n-swiper-initialized)").each(function () {
      this.classList.add('p2n-swiper-initialized');
      initSlider.bind(this)().then(swiper.bind(this)).then(function (swiper) {
        ...
        let webGel = null;
        if (this.classList.contains('p2n-webgl')) webGel = p2nGrid.WebGLDistortionHoverEffects({
          imgs: webGelOption.bind(this)(),
          ...(p2nGrid.getData(this, 'webgl', {}) || {}),
  ...
  function mouseCirMove($off) {
    const $element = $("#p2n_cursor"),
          inner = $("#p2n_cursor_inner");
    if (p2nParam.cursor.run) $body.addClass('p2n-cursor-effect');
    if (!$element.length || p2nGrid.isMobile() || !p2nParam.cursor.run) return;
    const mouseStop = 'p2n-stop-cursor';

    if (!$off) {
      p2nGrid.mouseMove($element, {
        speed: p2nParam.cursor.speed,
        mouseStop,
        inner: {
          el: inner,
          speed: p2nParam.cursor.speedInner
        }
      });
    }
    ...
    p2nGrid.mouseHover("a:not(> img):not(.vid) , .p2n-button-sidebar,  button , .button-load-more , [data-cursor=\"open\"]", {
    ...
    p2nGrid.mouseHover(".c-hidden , .social-side a , .next-arrow , .prev-arrow , .p2n-btn.vid", {
      enter() {
        stop();
        const {
          x,
  ...
  function list_project($el = $(document)) {
    ...
    $el.find('.woocommerce').each(($index, $item) => {
      const $grid = $($item).find('.p2n_grid');
      const $list = $($item).find('.p2n_list');
      if (!$grid.length) return;
      const $product = $($item).find('ul.p2n-shop');
...
function sidebarOptions() {
  document.body.classList.toggle('p2n-show-sidebar');
}